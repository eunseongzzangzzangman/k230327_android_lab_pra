...
class ImageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            ActivityResultContracts.StartActivityForResult())
        {
            try {
                ...
                Log.d("kkang", "inputStream 하고 난 후")
        ...
        val requestCameraFileLauncher = registerForActivityResult(
            ActivityResultContracts.StartActivityForResult()){
            //calRatio 임의로 만든 변수
            //이미지 처리시 크기 문제는 메모리 부족 현상이 생겼기 때문
            //크기를 조정 하는 함수
            //calculateInSampleSize 함수 : 저자가 임의로 만든 원본의 크기를 줄인 비율을 구하는 식
            ...
            Log.d("kkang", "원본의 사진을 줄이는데 어느 정도의 비율값(calRatio):$calRatio ")
            //BitmapFactory 비트맵 타입으로 이미지를 그대로 처리 시 문제가 됨! (OOM 메모리 누수)
            //옵션을 정해서, 이미지를 처리해야함
            val option = BitmapFactory.Options()
            //calRatio, 원본의 사진을 특정 비율에 맞게 줄인 결과의 값
        ...
        binding.cameraButton.setOnClickListener {
            ...
            //안드로이드 시스템에서 DIRECTORY_PICTURES 로 정해져 있음
            ...
            //카메라에서 찍은 사진에 접근 하기 위해, 콘텐츠 프로바이더에 요청
            //요청 시, 매니페스트에서 정한 같은 문자열을 사용
            ...
            //현재 앱 -> 외부 앱으로 가기위해, 시스템에게 인탠트로 전달
            //인탠트의 메시지 내용 - 액션의 문자열 카메라 앱
            //데이터의 내용 - 사진의 출력(카메라로 찍은 사진), photoURI에 담기
            val intent = Intent(MediaStore.ACTION_IMAGE_CAPTURE)
            intent.putExtra(MediaStore.EXTRA_OUTPUT, photoURI)
            //후처리 함수를 호출하면, 위의 후처리 작업하는 함수로 이동
            //카메라 촬영 후, 체크 한 다음, 되돌아 올 때 작업은 위의 함수에서 처리
    ...
    private fun calculateInSampleSize(fileUri: Uri, reqWidth: Int, reqHeight: Int): Int {
        ...
        //메모리 부족 현상해결을 위해 옵션이라는 속성 사용
        val options = BitmapFactory.Options()
        //실제 비트맵 객체를 생성하는 것이 아니고, 옵션만 설정하겠다하는 의미
        ...
        //사이즈가 여전히 크다면 원본의 사이즈를 반으로 줄이는 작업을 계속 진행
