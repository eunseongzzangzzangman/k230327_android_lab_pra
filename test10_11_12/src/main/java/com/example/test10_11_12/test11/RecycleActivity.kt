...
class RecycleActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 2) 어댑처 생성 : 기존 샘플 코드는 액티비티 내부 클래스로 구성
        // 예)MyAdapter
        // 저희는 따로 분리할 것.
        // 3) 설정한 리사이클러 뷰를 메인(RecycleActivity)에 적용한
        // 4) 임시 데이터(리스트), 원래 공공 데이터등 DB에서 받아온 데이터 사용

        // 임시 더미 데이터, 현재 텍스트 뷰만 20개 데이터
        ...
        // 리사이클러 뷰의 어댑터를 내가 만든 어댑터로 연결
        // 뷰와 데이터를 연결 해주는 역할
        binding.recyclerView.adapter = MyAdapter(datas)
        // 목록의 요소를 꾸며주는 역할. 이미지를 넣거나, 구분선을 넣거나,
        // 목록 요소의 크기 및 배치, 색깔 등을 정할 수 있음
