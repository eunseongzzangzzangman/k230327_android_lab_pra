    ...
    override fun onCreate(savedInstanceState: Bundle?) {
                ...
                MyApplication.auth.signInWithCredential(credential)
                    ...
                    .addOnCompleteListener(this){ task ->
                        if(task.isSuccessful){
                            //구글인증이 된 이메일의 현재 앱의 로그인된 email을 재할당하는 부분
                            MyApplication.email = account.email
                            //changeVisibility, 로그인 모드 마자게끔 뷰 구성
                            changeVisibility("login")
                        }else {
                            changeVisibility("logout")
                        }
        ...
        binding.googleLoginBtn.setOnClickListener {
            ...
            val gso = GoogleSignInOptions
                ...
                //인증 후, 안 보임. 이유? 인증된 아이디를 가져와 사용해서
        ...
        binding.signBtn.setOnClickListener {
            ...
            //인증 방법 중 이메일, 패스워드를 이용한 회원 가입 부분
            MyApplication.auth.createUserWithEmailAndPassword(email, password)
                //파이어베이스 인증 서비스에 이메일이 등록 됨 -> 인증 이메일 보내서 -> 이메일을 확인하면
                //등록된
                .addOnCompleteListener(this){
                    ...
                    if(task.isSuccessful){
                        //인증된 이메일이 존재한다면, 인증 메일을 등록할 메일로 전송
