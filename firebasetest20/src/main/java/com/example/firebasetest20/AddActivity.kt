...
class AddActivity : AppCompatActivity() {
    ...
    //전역으로 기본 바인딩, 뷰 객ㅊ 모두 모음
    lateinit var binding: ActivityAddBinding
    //파일 경로를 전역으로 설정해서, 갤러리에서 사진을 선택 한 후, 해당 파일의 절대 경로를 저장하는 파일
    lateinit var filePath: String

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        //바인딩 변환 후, 인플레이트를 이용하여, 출력 객체 초기화
    ...
    //사진 선택 후 돌아 왔을 때, 후처리를 하는 코드
    val requestLauncher = registerForActivityResult(
        ActivityResultContracts.StartActivityForResult())
    {
        //it 해당 정보를 담은 객체
        //안드로이드 버전의 ok, Http status 200과 동일한 기능
        if(it.resultCode === android.app.Activity.RESULT_OK){
            //가져온 이미지 처리를 글라이드를 이용
            Glide
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if(item.itemId === R.id.menu_add_gallery){
            ...
            //인텐트 옵션에서 액션 문자열은 이미지를 선택 후, URI를 가져오는 데이터 타입, MiME Type, 모든 이미지
            intent.setDataAndType(
                MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
                "image/*"
            )
            //인텐트의 후처리를 호출 하는 함수이고, 위에 정의한 변수로 이동함
            //사진을 선택해서 돌아오면 위의 코드로 이동함. ->
    ...
    private fun saveStore(){
        ...
        val data = mapOf(
            //인증된 유저의 이메일을 의미
            //앱 시작 시 인증을 체크하는 MyApplication의 checkAuth() 확인 함
        ...
        MyApplication.db.collection("news")
            ...
            .add(data)
                //파이어 스토어에 데이터를 저장을 잘 했다면, 동작하는 함수
            .addOnSuccessListener {
                //일반 데이터(문자열) 파이어 스토어 저장이 잘 되었을 때만
                //이미지를 스토리지에 저장하는 구조
                uploadImage(it.id)
            }
    ...
    private fun uploadImage(docId: String){
        //매개변수 부분은 글 작성시 docId 라고, 문서번호(자동생성)
        //add............................
        //MyApplication -> 스토리지를 사용하기 위한 객체
        val storage = MyApplication.storage
        //스토리지 객체에서 reference를 이용하여, 해당 객체를 바인딩
        val storageRef = storage.reference
        //imgRef 라는 객체로 업로드 및 다운로드를 실행하는데, 여기서는 업로드 부분
        //child -> 상위 폴더, images 하위에 이미지 파일이 저장되는 구조
        val imgRef = storageRef.child("images/${docId}.jpg")

        //후처리 코드에서 선택된 사진의 절대경로를 file 이라고 하는 참조형 변수에 할당
